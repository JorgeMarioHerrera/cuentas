...
import co.com.bancolombia.agremment.impl.AgremmentService;
...
class AgremmentServiceTest {
    ...
    @InjectMocks
    private AgremmentService deliveryService;
    ...
    void testShouldGetResponseFromDelivery() throws IOException {
        ...
        AgremmentService service = addProperties(okHttpClientMock, objectMapper, loggerApp);
        ...
        Either<ErrorExeption, ResponseAgremmentNit> result = service.callService(request, new RequestAgremmentFromFront());
        assertNotNull(result);
        assertTrue(result.isRight());
        assertFalse(result.isLeft());
        assertNotNull(result.getRight());
        assertNotNull(result.getRight().getAgreement());
        assertEquals(4,result.getRight().getAgreement().size() );
    }
    ...
    void testShouldGetResponseFromDeliveryEmptyBody() throws IOException {
        ...
        assertEquals(ErrorsEnum.AGR_ERR_BODY_NULL.buildError(), result.getLeft());
    }
    ...
    void testShouldSimulateIllegalStateExceptionWhenExecuteOkhttpClientInRetrieveBasic() throws IOException {
        ...
        assertTrue(result.isLeft());
        assertEquals(ErrorsEnum.AGR_ILLEGAL_STATE.buildError(),result.getLeft());
    }
    ...
    void testShouldSimulateJsonParseExceptionWhenExecuteOkhttpClient() throws IOException {
        ...
        Either<ErrorExeption, ResponseAgremmentNit> result = service.callService(request, new RequestAgremmentFromFront());
        assertTrue(result.isLeft());
        assertEquals(ErrorsEnum.AGR_JSON_PROCESSING.buildError(),result.getLeft());
    }
    ...
    void testShouldSimulateJsonMappingExceptionWhenExecuteOkhttpClient() throws IOException {
        ...
        assertTrue(result.isLeft());
        assertEquals(ErrorsEnum.AGR_ILLEGAL_ARGUMENT_EXCEPTION.buildError(),result.getLeft());
    }
    ...
    void testShouldSimulateIOExceptionWhenExecuteOkhttpClient() throws IOException {
        ...
        verify(objectMapper,times(0)).readValue(anyString(),eq(ResponseEntity.class));
        assertTrue(result.isLeft());
        assertEquals(ErrorsEnum.AGR_IO_EXCEPTION.buildError(),result.getLeft());
    }
    ...
    void testShouldGetErrorWhenConsumeCustomerDataAndHttpIs500() throws IOException {
        ...
        String codeExpect = Constants.CU_ERROR_PREFIX_AGREMMENT.concat(Factory.getResponseError().getErrors().get(0).getCode());
        // Create class with injections for constructor and set @Value of Spring
        AgremmentService service = addProperties(okHttpClientMock, objectMapper, loggerApp);
        UserTransactional request = UserTransactional.builder().dateAndHourTransaction("2021-09-22T09:11:33.497").sessionID("153556078799999-W96aWxsYS81LjA54758962700001").build();
        Either<ErrorExeption, ResponseAgremmentNit> result = service.callService(request, new RequestAgremmentFromFront());
    ...
    private AgremmentService addProperties(OkHttpClient okHttpClientMock, ObjectMapper objectMapper, LoggerApp loggerApp) {
        AgremmentService accountService= new AgremmentService(okHttpClientMock,objectMapper,loggerApp);
        ReflectionTestUtils.setField(accountService, "urlService", urlTest);
        ReflectionTestUtils.setField(accountService, "path", "/urlTest");
        ...
        ReflectionTestUtils.setField(accountService, "size", 10);
        ReflectionTestUtils.setField(accountService, "key", 1);
