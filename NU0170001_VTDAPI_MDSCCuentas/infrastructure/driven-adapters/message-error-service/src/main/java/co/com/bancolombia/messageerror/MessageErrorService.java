...
public class MessageErrorService implements IMessageErrorService {
    ...
    public Error obtainErrorMessageByAppIdCodeError(ErrorExeption error, String idSession) {
        loggerApp.init(this.getClass().toString(), LoggerOptions.Services.AC_DRIVEN_ADAPTER_MESSAGE_ERROR, idSession);
        try {
            loggerApp.logger(LoggerOptions.Actions.ACT_MESSAGE_ERROR_OBTAIN_ERROR, error.toString()
            ...
            loggerApp.logger(LoggerOptions.Actions.ACT_MESSAGE_ERROR_RESPONSE, response.toString(),
    ...
    private Response queryOKHttp(RequestMessageError requestMessageError) throws IOException {
        ...
        loggerApp.logger(LoggerOptions.Actions.ACT_MESSAGE_ERROR_QUERY, content,
                LoggerOptions.EnumLoggerLevel.DEBUG, null);
        RequestBody body = RequestBody.create(content, mediaType);
        Request request = new Request.Builder().url(urlService).post(body).build();
        loggerApp.logger(LoggerOptions.Actions.ACT_MESSAGE_ERROR_REQUEST, request.toString(),
                LoggerOptions.EnumLoggerLevel.DEBUG, null);
        // Ejecutando request
    ...
    private Error errorDefault(String codeErrorDefault, String technicalDescriptionErrorDefault) {
        loggerApp.logger(LoggerOptions.Actions.ACT_MESSAGE_ERROR_DEFAULT, null,
