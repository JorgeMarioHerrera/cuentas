package co.com.bancolombia.redis.impl;


import co.com.bancolombia.redis.template.UserTransactionalRedis;
import co.com.bancolombia.redis.util.Converter;
import co.com.bancolombia.redis.util.LoggerApp;
import co.com.bancolombia.redis.util.ErrorsEnum;
import co.com.bancolombia.redis.util.LoggerOptions;
import co.com.bancolombia.redis.util.Constants;
...
import co.com.bancolombia.redis.model.redis.gateways.IRedis;
import co.com.bancolombia.redis.repository.IRedisRepository;
...
public class RedisRepositoryImpl implements IRedis {
    ...
    public Either<ErrorExeption, Boolean> saveUser(UserTransactional user) {
        ...
        try {
            ...
            loggerApp.init(this.getClass().toString(), LoggerOptions.Services.VAL_DRIVEN_ADAPTER_REDIS,
                    user.getIdSession());
            iRedisRepository.save(Converter.modelToEntity(user));
    ...
    @Override
    public Either<ErrorExeption, List<UserTransactional>> getSessionsConcurrent(String numberDocument,
                                                                                String idSession) {
        Either<ErrorExeption, List<UserTransactional>> eitherResponse;
        try {
            validateField(numberDocument, "numberDocument");
            ...
            List<UserTransactionalRedis> listUserTransactionalRedis
                    = iRedisRepository.findByNumberDocument(numberDocument);
            listUserTransactionalRedis.forEach(resultRedis ->
                    listUserTransactional.add(Converter.entityToModel(resultRedis)));
    ...
    public Either<ErrorExeption, UserTransactional> getUser(String idSession) {
        ...
        try {
            ...
            eitherResponse = Either.right(Converter.entityToModel(iRedisRepository.findById(idSession)
            .orElse(UserTransactionalRedis.builder().build())));
